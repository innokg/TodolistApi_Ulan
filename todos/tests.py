...
class TodosAPITestCase(APITestCase): # родительский класс, который используется для наследования в тестах
...
class TestTodoDetailAPIView(TodosAPITestCase):
    ...
    def test_updates_one_item(self): # тестим на возможность апдейта
        self.authenticate()
        response = self.create_todo()
    ...
    def test_deletes_one_item(self): # тестим на возможность удаления
        self.authenticate()
